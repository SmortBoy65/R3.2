        ...
        for file in files:
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and (await db.get_bot_settings()).get('verify') == True:
                    ...
                    return
            if STREAM_MODE == True:
                button = [[InlineKeyboardButton('sᴛʀᴇᴀᴍ ᴀɴᴅ ᴅᴏᴡɴʟᴏᴀᴅ', callback_data=f'generate_stream_link:{file_id}')], [InlineKeyboardButton("Get Accurate Information", callback_data=f"get_info#{file_id}")]]
                reply_markup=InlineKeyboardMarkup(button)
            else:
                button = [[InlineKeyboardButton("Get Accurate Information", callback_data=f"get_info#{file_id}")]]
                reply_markup = InlineKeyboardMarkup(button)
