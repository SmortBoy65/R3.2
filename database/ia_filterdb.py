...
async def get_search_results(chat_id, query, file_type=None, max_results=10, offset=0, filter=False):
    ...
    if pattern_regex:
        ...
        final_filters = [match_criteria, {'file_name': {'$not': re.compile(r'\.(exe|avi)$', re.IGNORECASE)}}]
        if language:
            lang_filter = build_language_filter(language)
            if lang_filter:
                final_filters.append(lang_filter)

        match_criteria = {'$and': final_filters}
        ...
        pipeline.append({'$match': {'$and': final_filters}})
        ...
        try:
            ...
            if total_results == 0:
                ...
                final_filters = [filter_criteria, {'file_name': {'$not': re.compile(r'\.(exe|avi)$', re.IGNORECASE)}}]
                if language:
                    lang_filter = build_language_filter(language)
                    if lang_filter:
                        final_filters.append(lang_filter)

                filter_criteria = {'$and': final_filters}
...
def process_language_from_query(query):
    ...
    if "original" in query_parts:
        language = "original"
        language_token = "original"
    else:
...
def build_language_filter(language):
    ...
    if language == "original":
        all_lang_tokens = [token for lang in LANGUAGES.values() for token in lang]
        all_langs_regex_pattern = r'(?:^|[^a-zA-Z0-9])(' + '|'.join(map(re.escape, all_lang_tokens)) + r')(?:$|[^a-zA-Z0-9])'
        all_langs_regex = re.compile(all_langs_regex_pattern, re.IGNORECASE)
        return {'$nor': [{'file_name': all_langs_regex}, {'caption': all_langs_regex}]}
